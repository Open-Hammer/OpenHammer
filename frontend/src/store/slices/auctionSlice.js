...
const auctionSlice = createSlice({
  ...
  reducers: {
    ...
    getMyAuctionsRequest(state, action) {
      (state.loading = true), (state.myAuctions = []);
    },
    getMyAuctionsSuccess(state, action) {
      (state.loading = true), (state.myAuctions = action.payload);
    },
    getMyAuctionsFailed(state, action) {
      (state.loading = false), (state.myAuctions = []);
    },
...
export const getAllAuctionItems = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      "https://open-hammer.vercel.app/api/v1/auctionitem/allitems",
...
export const getAuctionDetail = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `https://open-hammer.vercel.app/api/v1/auctionitem/auction/${id}`,
      { withCredentials: true }
    );
...
export const createAuction = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      "https://open-hammer.vercel.app/api/v1/auctionitem/create",
      data,
      {
        withCredentials: true,
...
export const getMyActionItems = () => async (dispatch) => {
  dispatch(auctionSlice.actions.getMyAuctionsRequest());
  try {
    const response = await axios.get(
      "https://open-hammer.vercel.app/api/v1/auctionitem/myitems",
      { withCredentials: true }
    );
...
export const republishAuction = (id, data) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `https://open-hammer.vercel.app/api/v1/auctionitem/item/republish/${id}`,
      data,
      {
        withCredentials: true,
...
export const deleteAuction = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://open-hammer.vercel.app/api/v1/auctionitem/delete/${id}`,
      {
        withCredentials: true,
      }
    );
